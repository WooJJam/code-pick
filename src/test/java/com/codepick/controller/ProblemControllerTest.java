...
class ProblemControllerTest {
	...
	void searchProblems_AllParameters() throws Exception {
		...
		when(solvedAcService.searchProblems(anyString(), anyBoolean(), anyString(), anyList(), anyInt(), eq("asc"),
			eq(1), eq("id")))
	...
	void searchProblems_NoParameters() throws Exception {
		...
		when(solvedAcService.searchProblems(any(), any(), any(), any(), any(), any(), any(), any()))
			.thenReturn(mockResponse);

		// when & then
		mockMvc.perform(get("/api/v1/problems/search")
				.param("minSolvedCount", "0")
				.param("page", "1"))
			.andDo(print())
			.andExpect(status().isOk())
	...
	void searchProblems_OnlyUsername() throws Exception {
		...
		when(solvedAcService.searchProblems(eq("woojjam2"), anyBoolean(), isNull(), isNull(), isNull(), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
		...
		verify(solvedAcService).searchProblems("woojjam2", true, null, null, null, "asc",
			1, "id");
	}
	...
	void searchProblems_UnsolvedFalse() throws Exception {
		...
		when(solvedAcService.searchProblems(anyString(), eq(false), isNull(), isNull(), isNull(), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
		...
		verify(solvedAcService).searchProblems("woojjam2", false, null, null, null, "asc", 1, "id");
	}
	...
	void searchProblems_OnlyTierRange() throws Exception {
		...
		when(solvedAcService.searchProblems(isNull(), anyBoolean(), eq("g4..s3"), isNull(), isNull(), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
		...
		verify(solvedAcService).searchProblems(null, true, "g4..s3", null, null, "asc",
			1, "id");
	}
	...
	void searchProblems_MultipleTags() throws Exception {
		...
		when(solvedAcService.searchProblems(isNull(), anyBoolean(), isNull(), anyList(), isNull(), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
		...
		verify(solvedAcService).searchProblems(null, true, null, List.of("implementation", "dfs", "dp"), null, "asc",
			1, "id");
	}
	...
	void searchProblems_OnlyMinSolvedCount() throws Exception {
		...
		when(solvedAcService.searchProblems(isNull(), anyBoolean(), isNull(), isNull(), eq(10000), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
		...
		verify(solvedAcService).searchProblems(null, true, null, null, 10000, "asc",
			1, "id");
	}
	...
	void searchProblems_EmptyResult() throws Exception {
		// given
		SearchProblemResponse mockResponse = SearchProblemResponse.of(0, List.of());
		when(solvedAcService.searchProblems(anyString(), anyBoolean(), anyString(), isNull(), isNull(), eq("asc"),
			eq(1), eq("id")))
			.thenReturn(mockResponse);
	...
	void searchProblems_ComplexQuery() throws Exception {
		...
		when(
			solvedAcService.searchProblems(anyString(), anyBoolean(), anyString(), anyList(), anyInt(), eq("asc"),
				eq(1), eq("id")))
			.thenReturn(mockResponse);
